...
public class HSQLDBProperties {
    ...
    /**
     * Explicitly declare via property whether or not this HSQL database auto configuration should
     * run
     */
    ...
    /**
     * The working directory in the local file system where the database related files will be
     * stored, ending in a trailing slash
     */
    protected String workingDirectory = (System.getProperty("java.io.tmpdir") + File.separator
            + "broadleaf-hsqldb" + File.separator).replace("//", "/").replace("\\/", "\\");
    /**
     * Whether or not any database related files stored in the file system are wiped before launch.
     * This guarantees a fresh environment.
     */
    protected Boolean clearState = false;
    /**
     * Whether or not any database related files stored in the file system are wiped before launch.
     * Only true if the "clearStateProperty" and "clearStatePropertyValues" are confirmed.
     */
    protected Boolean clearStateOnPropertyOnly = true;
    /**
     * Used in conjunction with "clearStateOnPropertyOnly", defines the Spring environment property
     * to check.
     */
    protected String clearStateProperty =
            "blPU.hibernate.hbm2ddl.auto;spring.jpa.hibernate.ddl-auto";
    /**
     * Used in conjunction with "clearStateOnPropertyOnly", defines the value(s) [semicolon
     * delimited] that the "clearStateProperty" should be equal to to satisfy the condition. Leaving
     * this property empty signifies that any value for the property is accepted.
     */
